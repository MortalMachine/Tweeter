 ...
 */
public class PM2FollowGenerator {

    private static PM2FollowGenerator followGenerator;
     ...
     * @return the instance.
     */
    public static PM2FollowGenerator getInstance() {
        if(followGenerator == null) {
            followGenerator = new PM2FollowGenerator();
        }
    ...
    public List<Follow> generateUsersAndFollowers(User follower, int userCount, int minFollowersPerUser,
        ...
        List<User> users = PM2UserGenerator.getInstance().generateUsers(userCount);
        users.add(follower);
